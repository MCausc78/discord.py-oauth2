...
class MyClient(oauth2cord.Client):
    def __init__(self):
        super().__init__(intents=oauth2cord.Intents.default())
...
async def add(
    interaction: oauth2cord.Interaction,
...
async def channel_info(interaction: oauth2cord.Interaction, channel: Union[oauth2cord.VoiceChannel, oauth2cord.TextChannel]):
    """Shows basic channel info for a text or voice channel."""

    embed = oauth2cord.Embed(title='Channel Info')
    embed.add_field(name='Name', value=channel.name, inline=True)
    embed.add_field(name='ID', value=channel.id, inline=True)
    embed.add_field(
        name='Type',
        value='Voice' if isinstance(channel, oauth2cord.VoiceChannel) else 'Text',
        inline=True,
    )
...
async def shop(interaction: oauth2cord.Interaction, action: Literal['Buy', 'Sell'], item: str):
...
async def fruit(interaction: oauth2cord.Interaction, fruit: Fruits):
    """Choose a fruit!"""
...
class PointTransformer(app_commands.Transformer):
    async def transform(self, interaction: oauth2cord.Interaction, value: str) -> Point:
...
class Point3D(NamedTuple):
    ...
    @classmethod
    async def transform(cls, interaction: oauth2cord.Interaction, value: str):
        x, y, z = value.split(',')
        return cls(x=int(x.strip()), y=int(y.strip()), z=int(z.strip()))


@client.tree.command()
async def graph3d(interaction: oauth2cord.Interaction, point: Point3D):
    await interaction.response.send_message(str(point))
